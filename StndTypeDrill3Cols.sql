...
with t as (
Select
	grouping(${col1_to_grp}, ${col2_to_grp}, ${col3_to_grp}) result_rows,
	multiIf(result_rows > 0 and result_rows < pow(2,3)-1, 
					concat('Subtotal',toString(log2(result_rows+1))), 
					result_rows = pow(2,3)-1, 'Total', '    ') pivot,
	${col1_to_grp}, ${col2_to_grp}, ${col3_to_grp}
	,
	count() ct
from
	${schema}.${table} --final
	...
	--and ${col3_to_grp} = '${grp_to_filter3_val}'
...
--having pivot in ('    ', 'Total', 'Subtotal2')
order by
	result_rows desc,
	ct desc,
	${col1_to_grp} --desc
...
select
	*,
	row_number() over (partition by (pivot,	${col1_to_grp} ) order by ${col2_to_grp},${col3_to_grp}) as grpno_1_type,
	row_number() over (partition by (pivot,	${col1_to_grp}, ${col2_to_grp}) order by ${col3_to_grp}) as grpno_2_type
from
	t)
select
	pivot,
